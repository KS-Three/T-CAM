...
### Reversed a third time, then withdrawn the same day (settled 2026-09-09)

Kent asked again, with the thing the earlier proposals lacked: **a confidence,
and anything doubtful comes to him.** That was a real answer to the real
objection. The refusal above was never "a model cannot see antlers"; it was that
a wrong identity *silently* corrupts every movement pattern built on it, and a
confidence gate removes the word doing the damage. It was built on that basis:
`buck-match.mjs`, proposals only, stored as an unconfirmed `assist` claim beside
the vendor's claims, never auto-accepted at any confidence.

**Then he withdrew it, before it was ever wired into the review screen: he does
not want an API key.** The code is gone. §4's refusal stands, unamended.

Worth keeping, because it is the part that will come up again:

- **The confidence gate is a sound answer to the accuracy objection**, and it is
  what makes this reversible rather than reckless. If automated identity is ever
  revisited, start there and not from "the model is good enough now".
- **The blocker was never accuracy in the end. It was the key and the upload.**
  Every route to image comparison needs either a credential and photographs
  leaving the machine (§1, §6), or Python and model weights on it (§2, §10).
  Both are real costs to a tool whose selling points are no accounts, no install
  step, and your data staying on your PC. A future proposal has to answer that,
  not just the drift numbers.
- **What can still be built with neither**, and is not yet done: ranking the
  buck buttons in review by which bucks have actually been on that camera
  recently and at that hour, with the counts shown. §4 has called for ranked
  buttons since it was written, and `allBucks()` still returns them
  alphabetically. That is a prior derived from Kent's own records rather than a
  guess about pixels, it needs no key, and nothing leaves the machine.

**One thing could not be undone.** `MIGRATIONS` is append-only, so migration 19
stays: `detections.source` still permits `'assist'`. Nothing can write it now —
the code that would have is deleted — so it is an allowed value with no
producer. Deleting a shipped migration would leave a fresh database with a
different schema from one that had already run it, which is the exact drift the
append-only rule exists to prevent. The unused value is the cheaper of the two
problems.

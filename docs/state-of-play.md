...
| **Owner search** — find every parcel a name is on, statewide, largest first, capped at 50 | Live query against the Wisconsin service confirmed the ordering, the truncation flag, generalised boundaries and the second owner column; 13 module/API tests; 6 structural pins on the panel. **The panel itself has not been driven in a browser** — the Chrome extension was not connected in that session |
...
| **Weather bar redrawn** — the chip and bar float as glass over the map; the browser's slider is stripped to a dot on a groove, the week's rain drawn behind it (bar per hour, height = chance, darker = falling), a tick where now sits, pill buttons | Driven in headless Chrome against a live forecast on the fixture ground: 168 bars built with 15 wet, the now tick landing at 8.38% for hour 14 of 167, scrubbed Mon 2 pm → Thu 12 am, **ArrowLeft on the focused slider stepped to Wed 11 pm** (the reason the native input was kept rather than replaced by a div), Now returned; light, dark and 390 px all screenshotted. The first drive's 1 px bar gaps read as static rather than weather — closed to a continuous profile, screenshots of both in the PR. The `.wxbar`/layer-swatch overlap at phone width was measured on `origin/main` too (bar bottom 740, swatch top 712 in both) — pre-existing, untouched |
| **Radar loop** — the last two hours over the map from the weather bar, one track shared with the forecast via a Forecast/Radar switch; autoplay, scrub, Live, a live dot on the collapsed chip; blank past a 30-minute cutoff | Driven in headless Chrome against the LIVE service: 13 frames at 3 min old, autoplay advanced 2→5 unaided, a hand on the slider stopped it and reported "9 newer frames", Live returned to 12, and closing the bar left radar painting with the chip carrying the dot and the frame time. Palette screenshotted over live cells in New Mexico on satellite, topo and LiDAR grey. **The drive caught two that source tests could not**: the head reported the REEL's age (9 min) beside a frame 113 min old, and the service answers z8+ with an HTTP 200 "Zoom Level Not Supported" placard (identical 1370 bytes at z8 and z12) which tiled grey lettering across the property — radar is now capped at z7 and stretched, refused server-side above it. 35 module/API/UI tests |
...
| **This season from satellite** — a field's own NDVI curve from Sentinel-2, read through a zero-dependency COG reader (`cog.mjs`), saying whether the crop is standing, going off, or cut, and how old that answer is; crop identification exists but refuses on this ground | The COG reader driven against the real AWS bucket, not only its own fixtures: it reads NDVI 0.8268 where rasterio independently reads 0.8200, and the tests write their own TIFFs (Deflate, predictor, sparse and short tiles, a 200-instead-of-206 server) over a Range-serving stub. UTM pinned against PROJ to the millimetre and at the two places its definition fixes — and `check-crops.mjs` on Kent's own field immediately proved that decision earned its keep: the tile covering the property is zone **16** though its longitude says 15. A driven scan over stubbed imagery detects a harvest and leaves `fields.crop` and `cut_at` untouched. The Iowa separability measurement (corn 0.806 vs soy 0.406 on 19 Jun; 0.648 vs 0.909 on 28 Aug) is what the classifier rests on, and a 2 km CDL sample of Kent's ground — 40% wetland, 4% soybeans — is why it refuses here. Live run also caught that "standing" was 16 days stale behind cloud, now reported |
...
| **A new model that comes through blank says so** (`camera-shape.mjs`) — the extraction hunts by key name against the models on one account, so a differently-shaped document produces NULL for every field it cannot read. NULL means *unknown* everywhere downstream, which makes a misread camera and a silent one identical on the card, the pin and the health rules. The sync now names, per camera, the fields that arrived blank while the document appears to carry them, with the unread key's path. It never claims which key is the right one — it says the document mentions the thing and the sync took nothing. A field NOTHING in the document mentions is reported as unreported instead, and only the first kind prints on a normal run, because "this camera did not report a temperature" is true of healthy cameras every time. stderr, and survives `--quiet` | 19 unit tests plus 5 end-to-end, the latter running the real sync as a subprocess against a stand-in account carrying a model the extraction cannot read. Two traps found while writing it, both from driving the output rather than reading the source: the `last contact` probe matched `gps.lastFix.latDeg` and offered a LATITUDE as a candidate timestamp — a wrong answer is worse than none, so time probes now take only ISO-shaped values; and the `signal` probe missed a model calling it `reception`, reporting a sent field as never sent, which is the exact failure this exists to prevent (the probes are wide on purpose — being too narrow is the error that matters). The report is also printed with the CAMERA NAMED rather than "this camera", because stderr is often read on its own. 977 tests |
| **`--inspect` output is safe to send** — it dumps one camera PER MODEL (it took `cameras[0]`, which cannot show you the third camera's document — the exact case a new model on an existing account arrives as), and redacts by default. Coordinates, ids, SIMs and serials keep their type, precision and string FORMAT (`"N44 7.407360"` → `"A## #.######"`), which is what diagnosing a shape needs and carries no location; everything else is untouched, since a redacted battery percentage would defeat the point. `--raw` prints the true values and says so loudly | It previously printed the fix and asked the reader to trim it by hand — the trap this repo has already written down: writing the warning is not the same as obeying it. Pinned by a test asserting no line of a full FLEX-M dump contains the fix in ANY of its three forms (decimal pair, DMS minutes, geohash) while `status.model` and the battery percentage stay readable, and by an end-to-end assertion on the real sync's stdout. One judgement measured rather than assumed: the fix's own `dateTime` sits under a position path and is KEPT, because it is not a place and it is what says which fix is newest — but the keep-test is the strict ISO shape, not `Date.parse`, which reads "90.6" as June 1990 and would have handed back a low-precision coordinate string unredacted |
| **`camera-shape.cmd` / `Get-CameraShape.ps1`** — one double-click produces a REDACTED field-shape dump at `spypoint-data\shape.txt`, then reads its own output back and says whether the file is safe to send | Written after watching the by-hand route fail four ways in one sitting, each of which leaves an identical-looking file with no dump in it: run from the home directory and a loose old copy of the script answered; the example credentials from the instructions were pasted literally, so the login was rejected and nothing was dumped; the redirect pointed at the repo root, which is NOT gitignored; and no way to tell those apart afterwards. So the script sets its own location, reuses the launcher's DPAPI `.credentials.xml`, refuses outright when `camera-shape.mjs` is absent (an older checkout would print real coordinates), creates `spypoint-data/` itself — `--inspect` returns BEFORE the sync's mkdir, so on a fresh clone the redirect fails before node runs — and greps its own captured output for the redaction marker. **The PowerShell was not executed: this container is Linux and has no PowerShell.** Its idioms are copied from `Start-TrailCam.ps1`, which is known to work on Kent's machine, and the Node contract it depends on was verified here: the marker and `models on this account:` are on stdout, and a rejected login exits 1 having written no dump, which is the exact case that misled us. `$ErrorActionPreference` is relaxed around the node call on purpose — with it at Stop a native command writing to stderr can raise NativeCommandError, and this sync writes quota, staleness and blank-field notes there (13 such calls predate this work, so the launcher is already proven against it) |
| **FLEX-M2 reads correctly, and one vendor placeholder no longer becomes a fact** — the model that joined the account 2026-09-07. The fear was that a new model would come through blank; it does not, and `blanksFor()` says so on the real camera. What it did expose: its battery tray reports type `"UNK"` and it carries no `status.batteryType` at all, so the chain landed on the vendor's placeholder and stored a chemistry called UNK in the database, the CSV and the API. `UNK` / `UNKNOWN` / `N/A` / blank now normalize to null, per providers/README.md's own rule. `NA` is deliberately NOT on that list — a chemistry could be written that way, and turning a value into a null is the error nobody spots afterwards | Diagnosed from a REDACTED `--inspect` dump of the live account — the first real use of the tooling built earlier the same day, and it worked first time on Kent's machine including the PowerShell that could not be executed here. `fixtures/cameras.js` gained a FLEX_M2 built from that shape with invented ids and coordinates; 9 tests pin that every field reads, the [lng,lat] ordering (which this model makes easy to miss - it sends FOURTEEN decimal places where the FLEX-M sends six, so both numbers look unfamiliar), that the fix is stored as sent rather than rounded at ingest, and that a real chemistry like `AA` is untouched. Also cosmetic: the sync's own line printed `loc=44.12345678901234,...` - float noise dressed as precision - so `fmtLoc` now formats to six decimals like the camera card always has, guarded with isNum rather than a null check because `Number(null)` is 0 and 0,0 is in the Atlantic. 987 tests |
| **Correct a camera's pin** (migration 18) — a camera's lat/lng is the fix its own GPS reported, and a trail camera's GPS under a canopy is routinely 10-20 m out. Stands could be dragged; cameras could not, so a pin known to be wrong stayed wrong and everything measured from it inherited the error silently. `placed_lat`/`placed_lng`/`placed_at` hold the OWNER's answer BESIDE the device's, never on top: `cameraFromRow` returns the effective point as lat/lng so the pin, the cone and every distance follow without being told, while `gpsLat`/`gpsLng` keep the fix and the card reads `Pin — placed by hand, 38 m from the GPS fix`. `Use the GPS fix` undoes it, because a camera moved later makes yesterday's correction the wrong answer | Prompted by a field report (all six pins slightly off, 2026-09-08) that turned out NOT to be a bug: gps-doctor reported 0 yd between the stored row and all three encodings SpyPoint sends, the projection was proven exact (a pin lands on its tile corner to 15 dp; round-trip through pixelToLatLng is lossless), and the reported fix falls INSIDE the owner's own surveyed parcel 16 m from the west line, with the parcel boundary following the field edges in the imagery. So the tool was faithful and the missing thing was a way to say where the camera actually is. CONFIRMED afterwards by the reporter: SpyPoint's own app is off in the same way, which is the decisive test - two apps drawing one coordinate and both looking wrong means the coordinate is wrong. The report had originally said SpyPoint was correct, which is what sent this looking for an ingestion bug that did not exist; the two-app check is now the first thing the README tells you to do. 12 tests, the load-bearing one being that a correction SURVIVES A SYNC - the same trap `view` is guarded against, where the only symptom would be pins drifting back weeks later. Driven with a real mouse: armed, ground clicked, PATCH stored it, pin moved, card read 38 m, undo offered. gps-doctor now announces a corrected pin before its verdict, since every comparison it makes is about the GPS columns and the map is no longer drawing them - without that line it reads "nothing explains a wrong pin" to somebody whose pin is deliberately elsewhere. 999 tests |
| **Automated buck identification: built, then withdrawn (2026-09-09)** — `buck-match.mjs` asked a vision model which named buck a photograph showed and returned a proposal that was never auto-accepted. Kent asked for it, reversing design.md §4, then withdrew it the same day: he does not want an API key. The code and its 14 tests are deleted and §4's refusal stands | Nothing had been wired into the review screen, so nothing else referenced it and the removal was clean. MIGRATION 19 STAYS — `MIGRATIONS` is append-only, so `detections.source` still permits `'assist'` as an allowed value with no producer; deleting a shipped migration would give a fresh database a different schema from one that already ran it, which is the drift that rule prevents. The real blocker turned out not to be accuracy but the credential and the upload, and design.md records what a future attempt has to answer. Still open and needing neither: ranking the buck buttons by which bucks have been on that camera lately, which §4 has called for since it was written while `allBucks()` still returns them alphabetically |
| **The selection panel's buttons wrap** — a camera carrying both a facing and a corrected pin shows six buttons, and on one line the last of them (`Close`) sat 61 px outside the 330 px panel, reachable only by scrolling sideways. A flex item will not shrink below its content width, so nothing absorbed the sixth; the row now wraps, with an 88 px floor deciding where it breaks | Found by LOOKING at a screenshot, which is the only thing that showed it — every test passed with the button off the panel, and the row element's own box measured *inside* the panel while its last child hung outside. Measured in Chromium before and after against the panel's inner edge: `Close` at +61 px before, every one of the six at −1 px or better after, two rows of three at 95 px each; the panel scrolls 79 px to reach the second row on a short window, as it already did for the card, and at phone width it reads the same two-by-three. Pinned structurally in `test/map-ui.test.js`, and that pin verified to fail against the pre-fix source rather than passing vacuously. 1022 tests |
| **The statistics board replaces the report's prose** (`patterns.mjs`, `stats-board.mjs`, `/api/patterns`) — Kent, 2026-09-09: *"I don't like the AI generated camp report, I want more statistical analysis of the deer patterns."* "Best sits ahead" and "Where to sit" are gone from the dashboard; the alerts, the wind rose, review, the camera cards and the photographs stay. In their place: a scoreboard of cameras by rate per 100 camera-hours, small multiples of the 24-hour curve in camera-local **solar** time with the season's dawn and dusk drawn as the ranges they occupied, a camera × condition matrix cut by time of day / wind octant / temperature / barometer, and a card per named buck. Both provenances — your confirmed tags and the camera's unreviewed guesses — are computed separately, drawn side by side and **never summed**. The map shades its pins off the same payload, with a condition picker. `/api/stand-plan` and the Tonight page are untouched | The denominator is the load-bearing part: only `camera_days` rows whose state is `live` count, so a quota-dark camera and a camera watching an empty trail stop being indistinguishable. Verified by planting a known pattern in a 120-day generated season and requiring the module to find it — north-west wind ×3 at one camera came back **13.91 against a 3.85–5.10 baseline**, under-50°F ×2.6 came back 10.02/22.22 against 4.20/2.77, the dawn/dusk peak 18.06/16.67 against 1.81 by day, and the camera dark for 20 days held 2400 watched hours rather than 2880 and still ranked first. 21 unit tests, the last of which plants a 3× signal and requires it back at 2–4.5× while requiring the untouched temperature axis NOT to invent one. Driven in Chromium end to end: drawer opens, four scoreboard rows, four charts, all four axis buttons switch the matrix, the barometer axis flags itself tier D, neither replaced heading appears anywhere. **Two bugs only measurement found** — the activity bar was rendering entirely underneath the fixed top bar (`#map` is a stacking context at z-index 0, so nothing inside it can paint over z-index 20), and the halo was a tinted disc that measured 62px in the DOM and was invisible in the screenshot, dark green over dark green; it is a white ring with a shadow now, like every other mark this map puts over imagery. A third, `test/emitted-templates.test.js`, is a new guard: a backtick in a comment inside `mapStyles` closed the template early and the error pointed at a CSS property forty lines later. It happened twice in one sitting. 1052 tests |
...
| LiDAR terrain | Both, prominently | **Yes — free. Analysis reads USGS 3DEP at 1 m; since 2026-08-29 also a browsable LiDAR basemap and an over-imagery shade, like theirs, and since 2026-08-31 those draw from Wisconsin's own 2-ft county DEM with an exaggeration we control** |

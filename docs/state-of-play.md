...
| **`buck-match.mjs`** — asks a vision model which named buck a photograph shows, and returns a PROPOSAL. Kent's call 2026-09-09, reversing design.md §4 twice-settled refusal; the reasoning is in that entry. Nothing here writes: a proposal may only be stored as an UNCONFIRMED `assist` detection (migration 19, a third provenance beside `camera-ai` and `manual`), invisible to the ranking and to individuals.mjs until a person agrees. NO auto-accept at any confidence — Kent asked for a 50% gate and what shipped refuses to write at 99 too, because a confidently wrong match is exactly the case the original refusal was about | 14 tests against a stand-in API. Raw HTTP over global `fetch`, because the official SDK would end zero-dependency and the double-click launcher; the wire format is from the published Vision and Messages docs, not memory, and the request shape is asserted (labelled base64 image blocks, images before the question, a single tool, the three required headers). Every failure — refusal, 500, unreachable, no tool block, a name matching no buck — collapses to "no suggestion" with a reason, never a guess, and never throws, so a sync cannot fall over for it. **A bug my own test caught in my own code:** the confidence was read with `Number()`, and `Number(null)` is 0, so a missing confidence became a real "certainly not" — the trap this repo has now been bitten by four times. Type checked before conversion. **Photographs leave the machine**, which reverses design.md §1/§6; opt-in, off without a key, and only the animal's frame goes. **Not yet wired into the review screen** — that is the next step, and until then nothing calls it | 

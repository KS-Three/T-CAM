...
| **A camera can be placed by hand** — click its pin, Move, click where it really hangs. The vendor's fix is kept beside it ("its own GPS says 93 m away"), the sync never overwrites the correction, and "Use its own GPS" hands it back | Reported from the field 2026-08-30, *after* the newest-fix fix below — which was right and insufficient: the newest wrong answer is still wrong, and a cellular camera under canopy reports one routinely. 9 tests, the load-bearing one being that a later sync does not quietly undo it. Driven with a real mouse: pin clicked, Move pressed, ground clicked — the drive caught the coordinates saving and surviving a reload while **the pin never moved a pixel** (the pin list is filtered once at load and holds object references; a replaced object leaves every pin on the old copy). Re-driven: the pin lands on the clicked pixel, reload keeps it, GPS hand-back restores it |
...
- **The camera pin list is filtered once, at load.** `located` in `map-view.mjs`
  holds *references* to the objects in `D.cameras`, and the pins, the map
  framing and the ground clustering all read it. So a camera updated by
  assigning a fresh object into `D.cameras` still draws from the old one: the
  coordinates save, survive a reload, and the pin does not move. Mutate with
  `Object.assign` — the ground switcher's relabelling already does — and keep
  `located` in step when a camera gains or loses coordinates.
- **A camera with no GPS fix at all still cannot be placed.** It has no pin, so
  there is nothing to click, and the Move button lives on the pin's panel. Not
  hit yet (all four cameras report a fix); the fix would be a Move button on
  the camera's card in the Camp report drawer.

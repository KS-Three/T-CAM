...
import { groundsFrom, describeGround, resolveHomeGround } from './grounds.mjs';
...
export function createServer({ out = OPT.out } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      // where the map happens to be scrolled, and the stand suggester (since
      // removed) showed why that is not good enough: opened on its default "Everything" view
      ...
      if (req.method === 'GET' && url.pathname === '/api/my-properties') {
        ...
        for (let i = 0; i < grounds.length; i++) {
          ...
          out.push({
            ...
            // Rings are public record and already in memory from the lookup
            // above.
